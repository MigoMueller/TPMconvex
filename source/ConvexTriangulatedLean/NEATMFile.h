...
public:
	...
	struct etaPV; 
	etaPV fit(double accuracy=0.005) const;
	struct etaPV
	{
		double eta, pV;
	...
	{
		double value, sigma;
		ValueWithError(double value, double sigma) : value(value), sigma(sigma){};
	...
	struct dataLine
	{
		...
		operator const std::pair<const double, ValueWithError> () const 
			{return std::pair<const double, ValueWithError> (lambdaMu, ValueWithError(fluxSI, sigmaSI));};
	};	

	typedef std::multimap<double, ValueWithError> fluxMap;
