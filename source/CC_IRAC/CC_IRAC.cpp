...
int main(int nargs, char* argsv[])
{
  if (nargs != 5)
    {
      cerr<<"CC_IRAC, calculate IRAC color-correction factors for NEATM-like spectra"<<endl;
  ...
  const string bandpassFiles [] = {
    homeDir+"/tools/ch1.trimmed.txt",
    homeDir+"/tools/ch2.trimmed.txt",
    homeDir+"/tools/ch3.trimmed.txt",
    homeDir+"/tools/ch4.trimmed.txt"
  };
  ...
  cerr<<"# IRAC color-correction factors for NEATM spectrum with following parameters:"<<endl;
