...
const int    NEATMFile::nRefinements = 6;
 ...
 Sun2Aster(0,0), Earth2Aster(0,0)
{
	...
	while (!in.eof())
	{
		...
		{			
			...
			//data.insert(pair<double, ValueSigma> (lambdaMu, ValueSigma(flux, error)));
...
NEATMFile::etaPV NEATMFile::fit(double accuracy) const
{
	...
	etaPV midParms(etaMin+etaStep, pvDefault);
	double midChi2 = fitPV(asteroid, midParms, coarseAccuracy);
	etaPV rightParms (etaMin+2*etaStep, pvDefault);
	...
	for (int i=0; i<nRefinements; i++)
	{
		...
		{ // most likely minimum not on the 'right hand' side
			...
			double newChi2 = fitPV(asteroid, newParms, accuracy);
			if (newChi2 < midChi2)
			{ // new minimum on lhs
				rightParms=midParms; rightChi2=midChi2;
			...
			else
			{
				...
				double verynewChi2 = fitPV(asteroid, verynewParms, accuracy);
...
double NEATMFile::chi2(ConvexTriangulated &asteroid, double pV, double eta) const
{
	...
	while (it!=last)
	{
		...
		{
			const ValueSigma& fluxAndSuch=it->second;
...
double NEATMFile::fitPV(ConvexTriangulated& asteroid, double eta, double &pV, double accuracy) const
{
	...
	do
	{
		...
		pV/=sqrt(correction); // suppress overshooting around the 'true' minimum
		if (pV>pvMAX)
		{
			ostringstream dummy;
	...
	// apply sqrt(last correction) to estimate for chi^2 (don't be fooled, this is only a binomial performed!)
	return modelmodel*correction - 2*sqrt(correction)*modeldata + datadata;
}; // NEATMFile::fitPV
